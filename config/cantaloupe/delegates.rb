...
class CustomDelegate
  ...
  def httpsource_resource_info(options = {})
    file_string = context['identifier'].match(/_flysystem\/fedora\/(.*)/)
    if file_string
      # This one replaces the base domain with http://drupal
      #return { "uri" => "http://drupal/_flysystem/fedora/"+file_string[1] }
      # This one does the replacement and passes the auth headers. Note CAPITALIZATION MATTERS 'Authorization'
      if context['request_headers']['Authorization']
        return { "uri" => "http://drupal/_flysystem/fedora/"+file_string[1], "headers" => { "Authorization" => context['request_headers']['Authorization'] } }
      else
        return { "uri" => "http://drupal/_flysystem/fedora/"+file_string[1] }
      end
    else
      puts "Couldn't extract a value"
      return { "uri" => context['identifier' ] }
    end
  end
